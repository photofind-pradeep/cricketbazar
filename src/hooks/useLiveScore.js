...
// ═══════════════════════════════════════════════════════════════════
//  CricketBazar — Live Score + Fantasy Points Engine
//
//  PRIMARY  : /api/cricket (Netlify proxy → CricketData.org)
//             → Runs server-side, no "host not in allowlist" issue
//             → API key hidden from users
//
//  BACKUP   : cricbuzz-live.vercel.app (free, no key needed)
//             → Auto-used if proxy fails
//
//  SETUP (one time):
//  Netlify dashboard → Site settings → Environment variables → Add:
//  Key:   CRICKETDATA_KEY
//  Value: d7ef00e2-f81b-484e-abd8-f448f8d7b07b
// ═══════════════════════════════════════════════════════════════════

import { useState, useEffect, useCallback, useRef } from 'react'

// ─── API ENDPOINTS ───────────────────────────────────────────────
// Proxy URL — works in both local dev and production Netlify
const PROXY   = '/api/cricket'
const BACKUP  = 'https://cricbuzz-live.vercel.app/v1'
const REFRESH = 25000   // auto refresh every 25 seconds

// ─── DREAM11 EXACT T20 FANTASY SCORING ───────────────────────────
const T20 = {
  run: 1, four: 2, six: 3,
  fifty: 10, century: 20, duck: -5,
...
// Calculate full team points from live scorecard
...
const normalizeMatch = (m) => ({
  ...
  teams:  (m.teams || []).map((team, i) => ({
    team,
    run: m.score?.[i]
      ? `${m.score[i].r}/${m.score[i].w} (${m.score[i].o} ov)`
      : null,
  })),
  score: m.score || [],
})
...
const normalizeCricbuzz = (d) => ({
  bat1: [
    d.batsmanOne && {
      name: d.batsmanOne,
      r:    parseInt(d.batsmanOneRun  || 0),
      b:    parseInt((d.batsmanOneBall || '0').replace(/[()]/g, '')),
      sr:   parseFloat(d.batsmanOneSR || 0),
      '4s': 0, '6s': 0, dismissal: 'batting',
    },
    d.batsmanTwo && {
      name: d.batsmanTwo,
      r:    parseInt(d.batsmanTwoRun  || 0),
      b:    parseInt((d.batsmanTwoBall || '0').replace(/[()]/g, '')),
      sr:   parseFloat(d.batsmanTwoSR || 0),
      '4s': 0, '6s': 0, dismissal: 'batting',
    },
  ].filter(Boolean),
  bowl1: [
    d.bowlerOne && d.bowlerOne !== 'O' && {
      name: d.bowlerOne,
      o:    parseFloat(d.bowlerOneOver    || 0),
      r_c:  parseInt(d.bowlerOneRun       || 0),
      w:    parseInt(d.bowlerOneWickets   || 0),
      eco:  parseFloat(d.bowlerOneEconomy || 0),
      m: 0,
    },
    d.bowlerTwo && d.bowlerTwo !== 'O' && {
      name: d.bowlerTwo,
      o:    parseFloat(d.bowlerTwoOver    || 0),
      r_c:  parseInt(d.bowlerTwoRun       || 0),
      w:    parseInt(d.bowlerTwoWicket    || 0),
      eco:  parseFloat(d.bowlerTwoEconomy || 0),
      m: 0,
    },
  ].filter(Boolean),
  bat2: [], bowl2: [], hasTwo: false,
...
export function useLiveScore() {
  ...
  const fetchMatches = useCallback(async (tab = matchTab) => {
    ...
    // PRIMARY: Netlify proxy → CricketData.org (server side, no allowlist issue)
    try {
      ...
      setApiSource('CricketData.org ✅')
      setLoading(false)
      return
    } catch (e) {
      console.warn('[CricketBazar] Proxy failed, trying backup...', e.message)
    }

    // BACKUP: cricbuzz-live (free, no key)
    try {
      const res  = await fetch(`${BACKUP}/matches/${tab}?type=league`)
      if (!res.ok) throw new Error(`Backup HTTP ${res.status}`)
      const json = await res.json()
      const raw  = json.data?.matches || []
      setMatches(raw.map(m => ({
        id:     m.id,
        title:  m.title    || '',
        status: m.overview || '',
        isLive: tab === 'live',
        venue:  m.timeAndPlace?.place || '',
        teams:  m.teams || [],
        score:  [],
      })))
      setLastUpdated(new Date())
      setApiSource('cricbuzz-live (backup) ⚠️')
    } catch (e) {
      setError('Cannot reach any live score server. Check internet.')
      setMatches([])
  ...
  const fetchScorecard = useCallback(async (matchId) => {
    ...
    try {
      ...
      console.warn('[CricketBazar] Proxy scorecard failed, trying backup...', e.message)
    }

    // BACKUP: cricbuzz-live
    try {
      const res  = await fetch(`${BACKUP}/score/${matchId}`)
      const json = await res.json()
      setScorecard(normalizeCricbuzz(json.data || {}))
    } catch {
      setScorecard(null)
  ...
  // ── CONTROLS ─────────────────────────────────────────────
  const openMatch  = (m) => {
    setSelMatch(m); setScorecard(null); setActiveInning(0)
    fetchScorecard(m.id)
  }
  const closeMatch = () => { setSelMatch(null); setScorecard(null) }
  const refresh    = () => selMatch
    ? fetchScorecard(selMatch.id)
